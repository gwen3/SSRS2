...
--
-- Recherche date MAD Production positionnée après l'arrivée du chassis
,(select top 1 his.t_ddta from ttdsls451500 his 
				where his.t_orno =  lcde.t_orno and his.t_pono =  lcde.t_pono and 
						his.t_trdt > (select top 1 cha.t_rdat from twhltc500500 cha where cha.t_item = '         CHASSIS' and cha.t_serl = lcde.t_serl order by cha.t_rdat desc)) as DateMADProdArriveeChassis
-- Comparaison entre date MAD Production positionnée après l'arrivée du chassis et Date homologation ou Date d'achevement de l'OF
,(case
when (select top 1 cha.t_rdat from twhltc500500 cha where cha.t_item = '         CHASSIS' and cha.t_serl = lcde.t_serl and cha.t_cwar = lcde.t_cwar order by cha.t_rdat desc) IS NULL or
		(select top 1 cha.t_rdat from twhltc500500 cha where cha.t_item = '         CHASSIS' and cha.t_serl = lcde.t_serl and cha.t_cwar = lcde.t_cwar order by cha.t_rdat desc) < '01/01/1980'
	then NULL
else
	case when lastopof.t_rsdt IS NOT NULL
		then datediff(day, (select top 1 his.t_ddta from ttdsls451500 his 
				where his.t_orno =  lcde.t_orno and his.t_pono =  lcde.t_pono and 
						his.t_trdt > (select top 1 cha.t_rdat from twhltc500500 cha where cha.t_item = '         CHASSIS' and cha.t_serl = lcde.t_serl order by cha.t_rdat desc)),lastopof.t_rsdt)
		else
			case when ofab.t_cmdt is not NULL and ofab.t_cmdt > '01/01/1980'
				then datediff(day, (select top 1 his.t_ddta from ttdsls451500 his 
					where his.t_orno =  lcde.t_orno and his.t_pono =  lcde.t_pono and 
						his.t_trdt > (select top 1 cha.t_rdat from twhltc500500 cha where cha.t_item = '         CHASSIS' and cha.t_serl = lcde.t_serl order by cha.t_rdat desc)),ofab.t_cmdt)
				else NULL
			end
	end 
end) as TempsMADProductionFinFab
,(select count(*) from ttdsls401500 cptl inner join ttdsls411500 cpta on cpta.t_orno = cptl.t_orno and cpta.t_pono = cptl.t_pono and cpta.t_sqnb = cptl.t_sqnb and cpta.t_citg in (@citg)
				where cptl.t_orno = lcde.t_orno and cptl.t_item = lcde.t_item and cptl.t_serl = lcde.t_serl and cptl.t_clyn <> 1 and cptl.t_ddta between @ddta_f and @ddta_t) as CptArtSerie
,lcde.t_pono as LigneCde
-- Recherche Temps de la gamme auquel on applique ((Tps gamme /7) -- Temps gamme = temps prep
,(case 
when left(lcde.t_item, 9) = ' '
	then (case
	--gamme Standard
	when gam.t_stor = 1 then 
		(select (sum ((h.t_sutm*h.t_most)/60 + (h.t_rutm*h.t_mopr)/60)/7)  from ttirou102500 h
...
when left(lcde.t_item, 9) != ' '
	then (select (sum(prprj.t_nune)/7) from ttipcs360500 prprj where prprj.t_cprj = left(lcde.t_item, 9) and (prprj.t_cpcp like '%0100' or prprj.t_cpcp like '%0101'))
end) as LeadTimeProdObjectif
--
