----------------------------------------
-- LOG08 - Parametrage Flux Logistique
----------------------------------------
...
full join twhwmd302500 emp on emp.t_cwar = art.t_cwar and emp.t_item = art.t_item --Données d'Emplacement par Article
full join ttccom001500 nomemp on artplan.t_plid = nomemp.t_emno --Employés
inner join ttcibd001500 artgen on art.t_item = artgen.t_item --Articles 
where substring(art.t_item, 10, len(art.t_item)) between @art_f and @art_t --Bornage sur le numéro d'article sans code projet
