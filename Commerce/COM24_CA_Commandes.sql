-------------------------------------------
-- COM24 - CA Commandes par critères
-------------------------------------------
...
,dbo.convertlistcdf('tx','B61C','a9','ext','mav',lcde.t_cdf_moav, '4') as Motif_Avoir
...
where 
@invd_f = '01/01/1970'
and 'TOUT' in (@ttyp) --Test Type transaction financière
and lcde.t_odat between @prdt_f and @prdt_t --Bornage sur la Date de Commande
...
and cde.t_corg in (@corg_f) -- Origine de l'ordre
and cde.t_cofc in (@cofc_f) -- Services des ventes
and cde.t_crep between @crep_f and @crep_t --Représentant interne
and cde.t_osrp between @osrp_f and @osrp_t --Représentant Externe
and (select count(*) from ttdsls411500 tlcdeart where tlcdeart.t_orno = lcde.t_orno and tlcdeart.t_citg in (@citg_f)) >= 1
) union all
...
left outer join ttccom130500 adr on adr.t_cadr = cde.t_ofad --Adresses Commandes
left outer join ttccom130500 adrl on adrl.t_cadr = cde.t_stad --Adresses Livrées
where @invd_f = '01/01/1970' --Test si première date de la la plage "Date de facture" est 01/01/1970
and 'TOUT' in (@ttyp) --Test Type transaction financière
and lcde.t_odat between @prdt_f and @prdt_t --Bornage sur la Date de Commande
...
and (select count(*) from ttdsls411500 tlcdeart where tlcdeart.t_orno = lcde.t_orno and tlcdeart.t_citg in (@citg_f)) >= 1 --Au moins une ligne de la cde avec les groupes de la sélection
) union all 
...
,dbo.convertenum('td','B61C','a9','bull','sls.corg',prog.t_corg,'4') as OrigineOrdre
,'Non' as Bloquee
,'' as CodeSignal
,'' as Motif_Avoir
...
where @prgv_f = '1' -- Extraction des PV à Oui
and @invd_f = '01/01/1970'   -- Test si première date de facture est 01/01/1970
and 'TOUT' in (@ttyp) --Test Type transaction financière
and lprog.t_revn = (select top 1 lprog2.t_revn from ttdsls307500 lprog2 where lprog2.t_schn = lprog.t_schn and lprog2.t_sctp = lprog.t_sctp order by lprog2.t_revn desc) 
...
and lprog.t_odat between @prdt_f and @prdt_t --Bornage sur la Date de Commande
...
and prog.t_corg in (@corg_f) -- Origine de l'ordre
and prog.t_cofc in (@cofc_f) -- Services des ventes
and (select top 1 art.t_citg from ttcibd001500 art where art.t_item = lprog.t_item) in (@citg_f) --Groupes Articles
) union all 
...
where @prgv_f = '1' -- Extraction des PV à OUI
and (llivpv.t_ttyp in (@ttyp) or 'TOUT' in (@ttyp)) --Bornage type transaction financiere
and llivpv.t_stat between 9 and 10 --Bornage sur le statut qui doit être Facturé (9) ou Traité (10)
and llivpv.t_invd between @invd_f and @invd_t --Date de Facturation
and lprog.t_odat between @prdt_f and @prdt_t --Bornage sur la Date de Commande
and left(lprog.t_schn, 2) between @ue_f and @ue_t --Bornage sur l'UE défini par les Ordres Magasins
...
and prog.t_corg in (@corg_f) -- Origine de l'ordre
and prog.t_cofc in (@cofc_f) -- Services des ventes
and art.t_citg in (@citg_f) --Groupes Articles
) union all
...
left outer join ttccom130500 adrl on adrl.t_cadr = cde.t_stad --Adresses Livrées
where 
lcde.t_odat between @prdt_f and @prdt_t --Bornage sur la Date de Commande
and (livcde.t_ttyp in (@ttyp) or 'TOUT' in (@ttyp)) --Bornage type transaction financiere
and livcde.t_invd <> '01/01/1970' --Bornage sur les Dates de Facture qui ne sont pas vides, autrement dit la facture existe
and livcde.t_invd between @invd_f and @invd_t --Date de Facturation
and left(cde.t_cofc, 2) between @ue_f and @ue_t --Bornage sur l'UE
and cde.t_ofbp between @tiers_f and @tiers_t --Bornage sur le tiers acheteur
and cde.t_corg in (@corg_f) -- Origine de l'ordre
and cde.t_cofc in (@cofc_f) -- Services des ventes
