...
'' as QuantiteALivrer, 
'' as QuantiteLivreeNonFacturee, 
-- dboivin 22/06/2018 critère sur la société
...
'' as QuantiteLivreeNonFacturee, 
-- dboivin 22/06/2018 critère sur la société
(select sum(lfact.t_dqua) from tcisli310500 lfact where lfact.t_sfcp = lfac.t_sfcp and lfact.t_tran = fact.t_tran and lfact.t_idoc = fact.t_idoc) as QuantiteLivree, 
'' as DateLivraisonPlanifiee, 
'' as DateMADCPrevue, 
...
'' as ClientFinal,
-- dboivin 22/06/2018 critère sur la société
(select sum(lfact.t_amti) from tcisli310500 lfact where lfact.t_sfcp = lfac.t_sfcp and lfact.t_tran = fact.t_tran and lfact.t_idoc = fact.t_idoc) as MontantFacture, 
(select sum(lfact.t_amti) from tcisli310500 lfact where lfact.t_sfcp = fact.t_sfcp and lfact.t_tran = fact.t_tran and lfact.t_idoc = fact.t_idoc) as Montant, 
'0' as MontantAFacturer, 
fact.t_ccur as DeviseCommande, 
left(fact.t_fdpt, 2) as UE 
from tcisli305500 fact --En-tête de facture
...
'' as QuantiteALivrer, 
'' as QuantiteLivreeNonFacturee, 
-- dboivin 22/06/2018 ajout de la jointure sur la société
