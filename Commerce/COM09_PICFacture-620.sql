...
'' as ClientIndirect, 
'' as ClientFinal, 
(select sum(lfact.t_amti) from tcisli310620 lfact where lfact.t_idoc = lfac.t_idoc and lfact.t_tran = lfac.t_tran and lfact.t_ofbp = lfac.t_ofbp and lfact.t_srtp = 40 and lfact.t_koor = 7) as MontantFacture, 
(select sum(lfact.t_amti) from tcisli310620 lfact where lfact.t_idoc = lfac.t_idoc and lfact.t_tran = lfac.t_tran and lfact.t_ofbp = lfac.t_ofbp and lfact.t_srtp = 40 and lfact.t_koor = 7) as Montant, 
'0' as MontantAFacturer, 
fact.t_ccur as DeviseCommande, 
left(fact.t_fdpt, 2) as UE 
from tcisli305620 fact --Lignes de Commandes Clients
left outer join tcisli310620 lfac on lfac.t_tran = fact.t_tran and lfac.t_idoc = fact.t_idoc --Commandes Clients
...
fact.t_ccur as DeviseCommande, 
left(fact.t_fdpt, 2) as UE 
from tcisli310620 lfac --Lignes de Factures
inner join tcisli305620 fact on lfac.t_tran = fact.t_tran and lfac.t_idoc = fact.t_idoc --Factures
left outer join ttccom100500 tiers on tiers.t_bpid = lfac.t_ofbp --Tiers - Acheteur
...
'' as ClientIndirect, 
'' as ClientFinal, 
(select sum(lfact.t_amti) from tcisli310620 lfact where lfact.t_tran = fact.t_tran and lfact.t_idoc = fact.t_idoc) as MontantFacture, 
(select sum(lfact.t_amti) from tcisli310620 lfact where lfact.t_tran = fact.t_tran and lfact.t_idoc = fact.t_idoc) as Montant, 
'0' as MontantAFacturer, 
fact.t_ccur as DeviseCommande, 
left(fact.t_fdpt, 2) as UE 
from tcisli305620 fact --Lignes de Factures
inner join tcisli310620 lfac on lfac.t_tran = fact.t_tran and lfac.t_idoc = fact.t_idoc --Factures
left outer join ttccom100500 tiers on tiers.t_bpid = lfac.t_ofbp --Tiers - Acheteur
left outer join ttccom001500 empa on empa.t_emno = lfac.t_crep --Employés - Représentant Interne
left outer join ttcibd001500 art on art.t_item = lfac.t_item --Articles
...
'' as ClientIndirect, 
'' as ClientFinal, 
(select sum(lfact.t_amti) from tcisli310620 lfact inner join tcisli305620 factu on factu.t_tran = lfact.t_tran and factu.t_idoc = lfact.t_idoc where cast(factu.t_idat as date) = cast(fact.t_idat as date) and factu.t_itbp = fact.t_itbp and lfact.t_srtp = 30) as MontantFacture, 
(select sum(lfact.t_amti) from tcisli310620 lfact inner join tcisli305620 factu on factu.t_tran = lfact.t_tran and factu.t_idoc = lfact.t_idoc where cast(factu.t_idat as date) = cast(fact.t_idat as date) and factu.t_itbp = fact.t_itbp and lfact.t_srtp = 30) as Montant, 
'0' as MontantAFacturer, 
fact.t_ccur as DeviseCommande, 
left(fact.t_fdpt, 2) as UE 
from tcisli305620 fact --Lignes de Commandes Clients
